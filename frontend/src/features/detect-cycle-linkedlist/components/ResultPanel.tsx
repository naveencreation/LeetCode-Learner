...
export function ResultPanel({
  ...
  const completionMessage =
    currentStep === 0
      ...
      ? `Complete! Cycle ${hasCycle ? "detected" : "not detected"}.`
    ...
    <section className="traversal-panel grid h-full min-h-0 overflow-hidden grid-rows-[auto_minmax(0,1fr)] gap-1.5 p-2">
      ...
      <div className="ui-scrollbar min-h-0 overflow-y-auto pr-1">
        <div className="grid content-start gap-1.5">
          {/* Cycle Status + Phase cards */}
          <div className="grid grid-cols-2 gap-1.5">
            <div className={`rounded-lg border p-1.5 text-center text-white transition-all duration-200 ${
              hasCycle === true 
              ...
              <p className="text-[10px] font-bold uppercase tracking-[0.03em] opacity-90">
                Cycle Status
              </p>
              <p className="mt-0.5 text-xl font-extrabold">{hasCycle === true ? "Yes" : hasCycle === false ? "No" : "—"}</p>
            </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-2">
            ...
            <p className="mt-1 text-[12px] leading-relaxed text-slate-700">
              Floyd's algorithm uses two pointers moving at different speeds. If they meet, 
              a cycle exists. If fast reaches the end, there's no cycle. O(n) time, O(1) space.
            </p>
          </div>
