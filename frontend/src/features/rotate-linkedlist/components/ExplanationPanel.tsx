...
const EXPLANATION_BADGE_VARIANTS = {
  ...
  warning: { className: "border-amber-200 bg-amber-50 text-amber-700", icon: AlertTriangle },
  critical: { className: "border-rose-200 bg-rose-50 text-rose-700", icon: AlertCircle },
  ...
  if (!activeStep) {
    return (
      <section className="traversal-panel grid h-full min-h-0 overflow-hidden grid-rows-[auto_1fr] gap-2 p-2.5">
        <div className="traversal-panel-header"><h2 className="traversal-panel-title">Explanation</h2></div>
    ...
    <section className="traversal-panel grid h-full min-h-0 overflow-hidden grid-rows-[auto_1fr] gap-2 p-2.5">
      ...
      <div className="flex flex-col gap-3 overflow-auto">
        <div className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 ${variant.className}`}>
          <Icon size={14} /><span className="text-[10px] font-extrabold uppercase tracking-wide">{badge}</span>
        </div>
        ...
        {tip && (
          <div className="rounded-[10px] border border-sky-200 bg-sky-50 p-3">
            <p className="text-[10px] font-bold uppercase tracking-wide text-sky-600 mb-1">💡 Tip</p>
