...
function assignTopViewIndex(
  ...
  assignTopViewIndex(node.left, map, counter);
  map[node.val] = counter.value;
  counter.value += 1;
  assignTopViewIndex(node.right, map, counter);
}
...
function buildAutoPositions(
  ...
  const topviewIndex: Record<number, number> = {};
  assignTopViewIndex(root, topviewIndex, { value: 0 });
  ...
  nodes.forEach((node) => {
    ...
    positions[node.value] = {
      x: minX + topviewIndex[node.value] * xStep,
