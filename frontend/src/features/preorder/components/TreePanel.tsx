...
function assignPreorderIndex(
  ...
  assignPreorderIndex(node.left, map, counter);
  map[node.val] = counter.value;
  counter.value += 1;
  assignPreorderIndex(node.right, map, counter);
}
...
function buildAutoPositions(root: TreeNode | null): Record<number, NodePosition> {
  ...
  const preorderIndex: Record<number, number> = {};
  assignPreorderIndex(root, preorderIndex, { value: 0 });
  ...
  nodes.forEach((node) => {
    positions[node.value] = {
      x: minX + preorderIndex[node.value] * xStep,
