...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const visibleLines = useMemo(() => {
    ...
    const maxLine = Math.min(SYMMETRIC_CODE_LINES.length - 1, target + contextRadius);

    const result: Array<{ line: string; index: number }> = [];
    if (minLine > 0) {
      result.push({ line: "...", index: -1 });
    }
    for (let i = minLine; i <= maxLine; i += 1) {
      result.push({ line: SYMMETRIC_CODE_LINES[i], index: i });
    }
    if (maxLine < SYMMETRIC_CODE_LINES.length - 1) {
  ...
  const statusLabel = currentCodeLine > 0 
    ? (SYMMETRIC_LINE_LABELS[currentCodeLine] ?? "Code")
    : "Not started";
