...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const visibleLines = useMemo(() => {
    ...
    const maxLine = Math.min(BOUNDARY_CODE_LINES.length - 1, target + contextRadius);
    ...
    for (let i = minLine; i <= maxLine; i += 1) {
      result.push({ line: BOUNDARY_CODE_LINES[i], index: i });
    }
    if (maxLine < BOUNDARY_CODE_LINES.length - 1) {
  ...
  const statusLabel = currentCodeLine > 0 
    ? (BOUNDARY_LINE_LABELS[currentCodeLine] ?? "Code")
    : "Not started";
