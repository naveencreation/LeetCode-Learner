...
export function SharedCodePanel({
  ...
  const statusLabel = lineLabels[currentCodeLine] ?? "Context Line";

  // Pre-tokenize code lines for performance
  const tokenizedLines = useMemo(() => {
    return codeLines.map(tokenizeLine);
  }, [codeLines]);
  ...
  }, [currentCodeLine, executionLineNumbers, codeLines.length]);

  const visibleLines = useMemo(() => {
    if (viewMode === "full") {
      return codeLines.map((line, index) => ({ line, index }));
    }
    ...
    snippetLineIndices.forEach((index) => {
      ...
      rows.push({ line: codeLines[index], index });
      previousIndex = index;
    });

    return rows;
  }, [viewMode, snippetLineIndices, codeLines]);
    ...
    <section className="traversal-panel grid h-full min-h-0 grid-rows-[auto_1fr_auto] gap-2 p-2.5">
      <div className="traversal-panel-header">
        ...
        <div className="flex items-center gap-1.5">
          ...
          />
          <CopyCodeButton codeLines={codeLines} />
        </div>
      </div>
        ...
        >
          <code>
            {visibleLines.map(({ line, index }, rowIndex) => {
              ...
              const tokens = tokenizedLines[index] ?? tokenizeLine(line);
