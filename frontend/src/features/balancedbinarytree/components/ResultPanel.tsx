  ...
  const completionMessage =
    currentStep >= totalSteps
      ? result
        ...
        : "Traversal complete. Tree is unbalanced."
    ...
    <section className="traversal-panel grid h-full min-h-0 overflow-hidden grid-rows-[auto_minmax(0,1fr)] gap-1.5 p-2">
      ...
      <div className="ui-scrollbar min-h-0 overflow-y-auto pr-1">
        <div className="grid content-start gap-1.5">
          ...
          >
            {currentStep >= totalSteps ? (result ? <><CheckCircle2 size={14} className="inline shrink-0 text-emerald-600" />{" "}</> : <><XCircle size={14} className="inline shrink-0 text-rose-500" />{" "}</>) : <><ArrowRight size={14} className="inline shrink-0 text-amber-500" />{" "}</>}
