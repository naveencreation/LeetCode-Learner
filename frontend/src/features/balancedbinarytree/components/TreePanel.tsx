...
const stateStyles: Record<NodeVisualState,
...
const childByOperation: Partial<Record<ExecutionStep["type"], "left" | "right" | null>> = {
  check_left: "left",
  check_right: "right",
  enter_function: null,
  check_balance: null,
  return_height: null,
  return_unbalanced: null,
  exit_function: null,
};
      ...
      <div className="rounded-lg border border-teal-100 bg-teal-50 px-2.5 py-2 text-xs">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="max-w-[72%] truncate text-right font-extrabold text-teal-700">
            {sortedPositionEntries.length === 0 ? "Empty tree" : currentOperation}
          </span>
        </div>
      </div>
    </section>
