...
interface TreeSetupModalProps {
  ...
  selectedTargetP: number | null;
  selectedTargetQ: number | null;
  onClose: () => void;
  onApply: (
    root: TreeNode | null,
...
function normalizeTargets(
  ...
  const p = currentP !== null && values.has(currentP) ? currentP : sorted[0];
  const q =
    currentQ !== null && values.has(currentQ)
      ? currentQ
      : sorted.find((value) => value !== p) ?? p;
...
export function TreeSetupModal({
  ...
  const [draftTargetP, setDraftTargetP] = useState<number | null>(selectedTargetP);
  const [draftTargetQ, setDraftTargetQ] = useState<number | null>(selectedTargetQ);
  ...
  const currentNodeCount = availableValues.length;
  ...
  const hasUnsavedChanges =
    ...
    draftTargetP !== selectedTargetP ||
    draftTargetQ !== selectedTargetQ;
  ...
  const handlePresetChange = (preset: TreePresetKey) => {
    ...
    const normalized = normalizeTargets(presetRoot, null, null);

    setDraftPreset(preset);
    setDraftRoot(presetRoot);
    setDraftPositions({});
    setDraftTargetP(normalized.p);
    setDraftTargetQ(normalized.q);
    setError(null);
  };
  ...
  const handleAddNode = (side: "left" | "right") => {
    ...
    if (draftRoot === null) {
      ...
      setDraftTargetP(value);
      setDraftTargetQ(value);
      setNewValue("");
      setError(null);
      return;
    }
    ...
    if (draftTargetP === null) {
      setDraftTargetP(value);
    }
    if (draftTargetQ === null) {
      setDraftTargetQ(value);
    }
    setNewValue("");
    setError(null);
  ...
  const handleRenameNode = () => {
    ...
    if (draftTargetP === fromValue) {
      setDraftTargetP(toValue);
    }
    if (draftTargetQ === fromValue) {
      setDraftTargetQ(toValue);
    }
  ...
  const handleRemoveSubtree = (removeSide: "left" | "right") => {
    ...
    if (cloned.val === parentValueNumber) {
      ...
      const normalized = normalizeTargets(cloned, draftTargetP, draftTargetQ);
      setDraftTargetP(normalized.p);
      setDraftTargetQ(normalized.q);
  ...
  const handleApplyAction = (
    ...
    if (draftRoot && draftTargetP === null) {
      setError("Select target p.");
      return;
    }

    if (draftRoot && draftTargetQ === null) {
      setError("Select target q.");
      return;
    }

    applyHandler(draftRoot, draftPositions, draftPreset, draftTargetP, draftTargetQ);
    onClose();
  };
            ...
            <section className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              <div className="grid gap-2">
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-2.5">
                  ...
                  <div className="grid grid-cols-12 gap-1.5">
                    <label className="col-span-12 text-xs font-semibold text-slate-600 sm:col-span-6">
                      p node
                      <select
                        value={draftTargetP ?? ""}
                        onChange={(event) =>
                        ...
                        className="mt-1 h-9 w-full rounded-md border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-800 outline-none transition focus:border-teal-500"
                      >
                        {availableValues.length === 0 ? <option value="">N/A</option> : null}
                        {availableValues.map((value) => (
                          <option key={`target-p-${value}`} value={value}>
                            {value}
                          </option>
                        ))}
                      </select>
                    ...
                    <label className="col-span-12 text-xs font-semibold text-slate-600 sm:col-span-6">
                      ...
                      >
                        {availableValues.length === 0 ? <option value="">N/A</option> : null}
                        {availableValues.map((value) => (
                          <option key={`target-q-${value}`} value={value}>
                            {value}
                          </option>
                        ))}
                      </select>
