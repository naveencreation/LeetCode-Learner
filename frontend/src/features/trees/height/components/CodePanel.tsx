...
import { HEIGHT_CODE_LINES } from "../constants";
...
const HEIGHT_LINE_LABELS: Record<number, string> = {
  1: "Enter Function",
  2: "Base Case",
  3: "Recurse Left",
  4: "Recurse Right",
...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const statusLabel = HEIGHT_LINE_LABELS[currentCodeLine] ?? "Traversal Context";
  ...
  const snippetLineIndices = useMemo(() => {
    ...
    executionLineNumbers.forEach((lineNumber) => {
      for (let offset = -contextRadius; offset <= contextRadius; offset += 1) {
        ...
        if (index >= 0 && index < HEIGHT_CODE_LINES.length) {
          lineSet.add(index);
        }
      }
    });
  ...
  const visibleLines = useMemo(() => {
    if (viewMode === "full") {
      return HEIGHT_CODE_LINES.map((line, index) => ({ line, index }));
    }
    ...
    snippetLineIndices.forEach((index) => {
      ...
      rows.push({ line: HEIGHT_CODE_LINES[index], index });
      previousIndex = index;
    });
    ...
    <section className="traversal-panel grid h-full min-h-0 grid-rows-[auto_1fr_auto] gap-2 p-2.5">
      <div className="traversal-panel-header">
        ...
        <div className="flex items-center gap-1.5">
          ...
          />
          <CopyCodeButton codeLines={HEIGHT_CODE_LINES} />
        </div>
      </div>
