...
import { CopyCodeButton } from "@/features/trees/shared/components/CopyCodeButton";
import { SegmentedToggle } from "@/features/trees/shared/components/SegmentedToggle";

import { LEVEL_ORDER_CODE_LINES } from "../constants";
...
const LEVEL_ORDER_LINE_LABELS: Record<number, string> = {
  7: "Start Level Loop",
  8: "Read Level Size",
  9: "Initialize Level Bucket",
  11: "Pop Node From Queue",
  13: "Queue Left Child",
  15: "Queue Right Child",
  17: "Append Level To Answer",
};
...
function renderTokenizedCode(line: string | undefined): Array<{ text: string; className: string }> {
  const safeLine = line ?? "";
  ...
  const commentIndex = safeLine.indexOf("#");
  const codePart = commentIndex >= 0 ? safeLine.slice(0, commentIndex) : safeLine;
  const commentPart = commentIndex >= 0 ? safeLine.slice(commentIndex) : "";
...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const statusLabel = LEVEL_ORDER_LINE_LABELS[currentCodeLine] ?? "Traversal Context";
  ...
  const snippetLineIndices = useMemo(() => {
    ...
    executionLineNumbers.forEach((lineNumber) => {
      for (let offset = -contextRadius; offset <= contextRadius; offset += 1) {
        ...
        if (index >= 0 && index < LEVEL_ORDER_CODE_LINES.length) {
          lineSet.add(index);
        }
      }
    });
  ...
  const visibleLines = useMemo(() => {
    if (viewMode === "full") {
      return LEVEL_ORDER_CODE_LINES.map((line, index) => ({ line, index }));
    }
    ...
    snippetLineIndices.forEach((index) => {
      ...
      rows.push({ line: LEVEL_ORDER_CODE_LINES[index], index });
      previousIndex = index;
    });
    ...
    <section className="traversal-panel grid h-full min-h-0 grid-rows-[auto_1fr_auto] gap-2 p-2.5">
      <div className="traversal-panel-header">
        ...
        <div className="flex items-center gap-1.5">
          ...
          />
          <CopyCodeButton codeLines={LEVEL_ORDER_CODE_LINES} />
        </div>
      </div>
