...
const MAX_WIDTH_LINE_LABELS: Record<number, string> = {
  7: "Initialize Level Span",
  8: "Read First And Last Index",
  9: "Update Maximum Width",
  11: "Pop Node From Queue",
  13: "Queue Left Child Index",
  15: "Queue Right Child Index",
  17: "Return Maximum Width",
};
...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const statusLabel = MAX_WIDTH_LINE_LABELS[currentCodeLine] ?? "Traversal Context";
  ...
  const snippetLineIndices = useMemo(() => {
    ...
    executionLineNumbers.forEach((lineNumber) => {
      for (let offset = -contextRadius; offset <= contextRadius; offset += 1) {
        ...
        if (index >= 0 && index < MAX_WIDTH_CODE_LINES.length) {
          lineSet.add(index);
        }
      }
    });
  ...
  const visibleLines = useMemo(() => {
    if (viewMode === "full") {
      return MAX_WIDTH_CODE_LINES.map((line, index) => ({ line, index }));
    }
    ...
    snippetLineIndices.forEach((index) => {
      ...
      rows.push({ line: MAX_WIDTH_CODE_LINES[index], index });
      previousIndex = index;
    });
