...
import { BSTDLL_TREE_PRESETS, cloneTree, createSampleTree } from "./constants";
import { generateBstdllExecutionSteps } from "./engine";
...
interface BstdllTraversalReturn {
  root: TreeNode | null;
...
export function useBstdllTraversal(): BstdllTraversalReturn {
  // Memoize config to avoid unnecessary recalculations
  const config = useMemo(
    () => ({
      generateSteps: generateBstdllExecutionSteps,
      presets: BSTDLL_TREE_PRESETS,
      cloneTree,
      createSampleTree,
      getCodeLineForStep,
      getOperationBadge,
  ...
  return useGenericTraversal(config) as BstdllTraversalReturn;
}
