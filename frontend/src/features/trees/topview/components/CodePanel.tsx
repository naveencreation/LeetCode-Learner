...
const TOP_VIEW_LINE_LABELS: Record<number, string> = {
  6: "Start While Loop",
  7: "Pop Queue Head",
  8: "Check First Horizontal Distance",
  9: "Capture Top View Node",
  11: "Queue Left Child",
  13: "Queue Right Child",
};
...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const statusLabel = TOP_VIEW_LINE_LABELS[currentCodeLine] ?? "Traversal Context";
  ...
  const snippetLineIndices = useMemo(() => {
    ...
    executionLineNumbers.forEach((lineNumber) => {
      for (let offset = -contextRadius; offset <= contextRadius; offset += 1) {
        ...
        if (index >= 0 && index < TOP_VIEW_CODE_LINES.length) {
          lineSet.add(index);
        }
      }
    });
  ...
  const visibleLines = useMemo(() => {
    if (viewMode === "full") {
      return TOP_VIEW_CODE_LINES.map((line, index) => ({ line, index }));
    }
    ...
    snippetLineIndices.forEach((index) => {
      ...
      rows.push({ line: TOP_VIEW_CODE_LINES[index], index });
      previousIndex = index;
    });
