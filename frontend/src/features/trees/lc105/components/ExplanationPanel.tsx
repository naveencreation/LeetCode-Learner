import { LC105_LINE_GUIDE } from "../constants";
...
export function ExplanationPanel({
  ...
  const guide = LC105_LINE_GUIDE[currentCodeLine];
  const isDone = currentStep >= totalSteps;

  const title = isDone
    ? "Construction Complete"
    : `Line ${currentCodeLine + 1}: ${activeStep?.type ?? "build context"}`;

  const description = isDone
    ? `All build steps are complete. Root creation order is [${createdOrder.join(", ")}].`
    : activeStep?.operation ??
      'Click "Next Step" to start and follow preorder root picks with inorder splits.';
      ...
      <div className="min-h-0 space-y-2 overflow-auto rounded-[10px] border border-sky-200 bg-gradient-to-b from-cyan-50 to-sky-50 p-2">
        ...
        <ul className="grid gap-1 text-[11px]">
          {guide ? (
            ...
            <li className="rounded-lg border border-sky-200 bg-white/80 px-2 py-1 text-cyan-900">
              &gt; Build helper uses inorder boundaries to determine when recursion should stop.
            </li>
          )}
          <li className="rounded-lg border border-sky-200 bg-white/80 px-2 py-1 text-cyan-900">
            &gt; Progress snapshot: {currentStep}/{totalSteps} steps, {createdOrder.length} nodes created.
          </li>
        </ul>
      </div>
