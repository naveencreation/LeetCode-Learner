...
import { PRE_IN_POST_CODE_LINES, PRE_IN_POST_LINE_LABELS } from "../constants";
...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const statusLabel = PRE_IN_POST_LINE_LABELS[currentCodeLine] ?? "Context Line";
  ...
  const snippetLineIndices = useMemo(() => {
    ...
    executionLineNumbers.forEach((lineNumber) => {
      for (let offset = -contextRadius; offset <= contextRadius; offset += 1) {
        ...
        if (index >= 0 && index < PRE_IN_POST_CODE_LINES.length) {
          lineSet.add(index);
        }
      }
    });
  ...
  const visibleLines = useMemo(() => {
    if (viewMode === "full") {
      return PRE_IN_POST_CODE_LINES.map((line, index) => ({ line, index }));
    }
    ...
    snippetLineIndices.forEach((index) => {
      ...
      rows.push({ line: PRE_IN_POST_CODE_LINES[index], index });
      previousIndex = index;
    });
    ...
    <section className="traversal-panel grid h-full min-h-0 grid-rows-[auto_1fr_auto] gap-2 p-2.5">
      <div className="traversal-panel-header">
        ...
        <div className="flex items-center gap-1.5">
          ...
          />
          <CopyCodeButton codeLines={PRE_IN_POST_CODE_LINES} />
        </div>
      </div>
