...
import { LC114_CODE_LINES, LC114_LINE_LABELS } from "../constants";
...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const snippetLineIndices = useMemo(() => {
    ...
    executionLineNumbers.forEach((lineNumber) => {
      for (let offset = -contextRadius; offset <= contextRadius; offset += 1) {
        ...
        if (index >= 0 && index < LC114_CODE_LINES.length) {
          lineSet.add(index);
        }
      }
    });
  ...
  const visibleLines = useMemo(() => {
    if (viewMode === "full") {
      return LC114_CODE_LINES.map((line, index) => ({ line, index }));
    }
    ...
    snippetLineIndices.forEach((index) => {
      ...
      rows.push({ line: LC114_CODE_LINES[index], index });
      previousIndex = index;
    });
  ...
  const statusLabel = LC114_LINE_LABELS[currentCodeLine] ?? "Context Line";
    ...
    <section className="traversal-panel grid h-full min-h-0 grid-rows-[auto_1fr_auto] gap-2 p-2.5">
      <div className="traversal-panel-header">
        ...
        <div className="flex items-center gap-1.5">
          ...
          />
          <CopyCodeButton codeLines={LC114_CODE_LINES} />
        </div>
      </div>
