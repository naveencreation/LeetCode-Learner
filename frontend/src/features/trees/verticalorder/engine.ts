...
export function generateTopViewExecutionSteps(root: TreeNode | null): {
  ...
  if (!root) {
    executionSteps.push({
      ...
      hd: 0,
      level: 0,
      operation: "Tree is empty; vertical order is []",
      callStack: [],
      nodeStates: cloneNodeStates(nodeStates),
    });
  ...
  while (queue.length > 0) {
    ...
    pushStep(
      ...
      hd,
      level,
      `Pop node ${node.val} (row=${level}, col=${hd}) from queue`,
    ...
    pushStep(
      ...
      hd,
      level,
      `Append node ${node.val} into col ${hd} at row ${level}`,
      getCallStackSnapshot(callStack, "executing", frameId),
      nodeStates,
    );
    ...
    if (node.left) {
      ...
      pushStep(
        ...
        hd,
        level,
        `Queue left child ${node.left.val} with row ${level + 1}, col ${hd - 1}`,
        getCallStackSnapshot(callStack, "executing", frameId),
        nodeStates,
      );
    ...
    if (node.right) {
      ...
      pushStep(
        ...
        hd,
        level,
        `Queue right child ${node.right.val} with row ${level + 1}, col ${hd + 1}`,
        getCallStackSnapshot(callStack, "executing", frameId),
        nodeStates,
      );
