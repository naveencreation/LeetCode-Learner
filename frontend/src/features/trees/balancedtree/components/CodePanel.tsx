...
import { BALANCED_CODE_LINES, BALANCED_LINE_LABELS } from "../constants";
...
export function CodePanel({ currentCodeLine, executionLineNumbers }: CodePanelProps) {
  ...
  const visibleLines = useMemo(() => {
    ...
    if (target <= 0) {
      return BALANCED_CODE_LINES.map((line, index) => ({ line, index }));
    }
    const minLine = Math.max(0, target - contextRadius);
    const maxLine = Math.min(BALANCED_CODE_LINES.length - 1, target + contextRadius);
    const result: Array<{ line: string; index: number }> = [];
    if (minLine > 0) result.push({ line: "...", index: -1 });
    for (let i = minLine; i <= maxLine; i++) {
      result.push({ line: BALANCED_CODE_LINES[i], index: i });
    }
    if (maxLine < BALANCED_CODE_LINES.length - 1) result.push({ line: "...", index: -2 });
    return result;
  }, [currentCodeLine, contextRadius]);

  const statusLine = currentCodeLine > 0 ? currentCodeLine : "—";
  const statusLabel = currentCodeLine > 0 ? (BALANCED_LINE_LABELS[currentCodeLine] ?? "Code") : "Not started";
    ...
    <section className="flex h-full flex-col gap-1.5 overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          />
          <CopyCodeButton codeLines={BALANCED_CODE_LINES} />
        </div>
      </div>
