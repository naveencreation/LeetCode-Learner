...
import { createLinkedList, type ListNode } from "@/features/linked-lists/shared/linked-list-types";

interface ListSetupModalProps {
  onClose: () => void;
  onApply: (listA: ListNode | null, listB: ListNode | null) => void;
  onApplyAndRun: (listA: ListNode | null, listB: ListNode | null) => void;
}

const DEFAULT_LIST1 = "1, 2, 3, 4";
...
export function ListSetupModal({
  ...
  const handleApply = () => {
    ...
    // Create intersection at the last node of list1
    const common = createLinkedList([values1[values1.length - 1]]);
    const list1 = createLinkedList(values1.slice(0, -1));
    let curr = list1;
    while (curr?.next) curr = curr.next;
    if (curr) curr.next = common;

    const list2 = createLinkedList(values2.slice(0, -1));
    curr = list2;
    while (curr?.next) curr = curr.next;
    if (curr) curr.next = common;
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <div className="mb-4 rounded-lg border border-sky-200 bg-sky-50 p-3">
          <p className="text-xs text-sky-700">
            Note: Lists will automatically intersect at the last unique node of List A for visualization purposes.
          </p>
        </div>
