...
import {
  ...
  type ListNode,
} from "@/features/linked-lists/shared/linked-list-types";
...
function MiniChainPreview({ values, cyclePosition }: { values: number[]; cyclePosition: number | null }) {
    ...
    <div className="flex items-center gap-1 flex-wrap">
      {values.map((val, i) => (
        <div key={val} className="flex items-center">
          <div className={`flex h-6 w-8 items-center justify-center rounded border text-[10px] font-bold ${
            cyclePosition !== null && i === values.length - 1
      ...
      {cyclePosition !== null && (
        <span className="ml-1 text-[10px] text-rose-500">→ (cycle to {values[cyclePosition]})</span>
      )}
      {cyclePosition === null && (
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <InfoTip text="Choose a preset or enter custom values. Optionally add a cycle to test cycle detection." />
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="max-h-[60vh] overflow-y-auto p-4">
          ...
          <div className="mb-4">
            ...
            >
              ...
              {isCustom && customValues && (
                <div className="mt-2">
                  <MiniChainPreview values={customValues} cyclePosition={hasCycle ? cyclePosition : null} />
                </div>
              )}
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2">
                ...
                />
                <span className="text-xs font-medium text-slate-700">Add Cycle</span>
              </label>
              <InfoTip text="When enabled, the last node will point back to create a cycle for testing Floyd's algorithm." />
            </div>
          ...
          {!isCustom && previewValues.length > 0 && (
            <div className="rounded-lg border border-slate-100 bg-slate-50 p-3">
              ...
              <MiniChainPreview values={previewValues} cyclePosition={hasCycle ? cyclePosition : null} />
            </div>
          )}
        </div>
