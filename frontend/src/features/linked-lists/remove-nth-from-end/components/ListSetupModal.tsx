...
interface ListSetupModalProps {
  ...
  currentN: number;
  onClose: () => void;
  onApply: (head: ListNode | null, preset: LinkedListPresetKey, n: number) => void;
  onApplyAndRun: (head: ListNode | null, preset: LinkedListPresetKey, n: number) => void;
}
...
export function ListSetupModal({
  ...
  const handleApply = () => {
    ...
    const finalN = isNValid ? nValue : Math.min(nValue || 2, values.length);
    onApply(head, isCustom ? "medium" : activePreset, finalN);
    onClose();
  };
  ...
  const handleApplyAndRun = () => {
    ...
    const head = createLinkedList(values);
    const finalN = isNValid ? nValue : Math.min(nValue || 2, values.length);
    onApplyAndRun(head, isCustom ? "medium" : activePreset, finalN);
    onClose();
  };
        ...
        <div className="max-h-[60vh] overflow-y-auto p-4">
          ...
          <div className="mb-4">
            <div className="mb-2 flex items-center gap-2">
              ...
              <InfoTip text="The position from the end to remove (1 = last node)" />
            </div>
            <div className="rounded-lg border border-slate-200 bg-white p-3">
              <input
                ...
                className="w-full rounded border border-slate-200 px-3 py-2 text-sm font-medium text-slate-800 focus:border-violet-400 focus:outline-none"
              />
              <p className="mt-1.5 text-[10px] text-slate-500">
                Remove the {nValue}{nValue === 1 ? "st" : nValue === 2 ? "nd" : nValue === 3 ? "rd" : "th"} node from the end.
                {listLength > 0 && ` Valid range: 1 to ${listLength}`}
              </p>
              {!isNValid && listLength > 0 && (
                <p className="mt-1 text-[10px] font-medium text-rose-600">
                  Invalid n value. Must be between 1 and {listLength}.
                </p>
              )}
            </div>
          </div>
