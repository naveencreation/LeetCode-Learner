...
import { LinkedListSVG } from "@/features/linked-lists/shared/components/LinkedListSVG";
...
interface LinkedListPanelProps {
  ...
  currentPhase: string;
  rotateK: number;
  setRotateK: (k: number) => void;
  onOpenListSetup: () => void;
}

export function LinkedListPanel({
  originalValues, nodeStates, activeStep, currentOperation, currentStep, totalSteps, currentPhase, rotateK, setRotateK, onOpenListSetup,
}: LinkedListPanelProps) {
  const pointers = {
    prev: null as number | null,
    curr: activeStep?.pointers.curr ?? null,
    ...
    <section className="traversal-panel grid h-full min-h-0 overflow-hidden grid-rows-[auto_auto_1fr] gap-2 p-2.5">
      ...
      <div className="grid grid-cols-4 gap-1.5 rounded-[10px] border border-slate-200 bg-white/85 p-1.5">
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-2 py-1 text-center">
          <p className="text-[9px] font-bold uppercase tracking-wide text-amber-600">K</p>
          <p className="text-[11px] font-extrabold text-amber-800">{rotateK}</p>
        </div>
      </div>
      <div className="rounded-[10px] border border-slate-200 bg-gradient-to-b from-[#f8faff] to-[#f0f4fb] p-3">
        <LinkedListSVG values={originalValues} nodeStates={nodeStates} links={activeStep?.links ?? {}} pointers={pointers} />
      </div>
    </section>
  );
}
