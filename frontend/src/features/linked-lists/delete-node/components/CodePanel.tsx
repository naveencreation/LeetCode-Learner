...
import { PYTHON_CODE, PYTHON_CODE_LINES, LINE_LABELS } from "../constants";
        ...
        <pre ref={preRef} className="font-[var(--font-jetbrains)] text-[11px] leading-[1.6]">
          {lines.map((line, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`flex gap-2 ${isCurrentLine ? "bg-slate-800" : ""} ${isExecutionLine && !isCurrentLine ? "bg-slate-800/40" : ""}`}
              >
