...
export function ResultPanel({
  ...
  const completionMessage =
    currentStep >= totalSteps
      ? `Done. Final diameter is ${currentDiameter}.`
  ...
  return (
    <section className="traversal-panel grid h-full min-h-0 overflow-hidden grid-rows-[auto_minmax(0,1fr)] gap-1.5 p-2">
      <div className="traversal-panel-header">
        <h2 className="traversal-panel-title">Diameter Progress</h2>
      </div>
        ...
        <div className="grid content-start gap-1.5">
          <div className="grid grid-cols-3 gap-1.5">
            ...
            <div className="rounded-lg border bg-gradient-to-br from-teal-700 to-teal-500 p-1.5 text-center text-white">
              ...
              <p className="mt-0.5 truncate text-sm font-extrabold">{currentPhase}</p>
            </div>
            <div className="rounded-lg border bg-gradient-to-br from-emerald-700 to-emerald-500 p-1.5 text-center text-white">
              <p className="text-[10px] font-bold uppercase tracking-[0.03em] text-emerald-100">Best Diameter</p>
              <p className="mt-0.5 text-xl font-extrabold">{currentDiameter}</p>
            </div>
          </div>

          <div className="grid gap-1 rounded-lg">
            <p className="text-[11px] font-extrabold uppercase tracking-[0.03em] text-slate-700">Diameter Timeline</p>
            <div className="flex min-h-[36px] flex-wrap items-center gap-1.5 rounded-lg border border-slate-200 bg-white p-1.5">
              {diameterHistory.length === 0 ? (
                <span className="text-[11px] text-slate-400">Diameter updates appear here...</span>
              ) : (
                diameterHistory.map((value, index) => (
                  <span
                    key={`${value}-${index}`}
                    className="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-emerald-700 to-emerald-500 px-2.5 py-1 text-xs font-extrabold text-white"
                  >
                    {value}
                  </span>
                ))
