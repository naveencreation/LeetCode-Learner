...
function buildProjectionStepState(
  ...
  for (let index = 0; index < currentStep; index += 1) {
    ...
    if (
      ...
      const bucket = visibleByHd.get(step.hd) ?? [];
      bucket.push(step.value);
      visibleByHd.set(step.hd, bucket);
        ...
        {viewMode === "tree" ? (
          ...
          <svg viewBox={`0 0 ${viewWidth} ${viewHeight}`} className="h-full w-full">
            ...
            {projectionColumns.map((column) => {
              const capturedValues = projectionStepState.visibleByHd.get(column.hd) ?? [];
              const visibleValue = capturedValues[capturedValues.length - 1];
              const visibleNode =
              ...
              const hiddenNodes = column.nodes
                .filter((node) => !capturedValues.includes(node.value))
                .sort((a, b) => {
                  if (a.depth !== b.depth) {
                    return a.depth - b.depth;
                  }
      ...
      <div className="rounded-lg border border-teal-100 bg-teal-50 px-2.5 py-2 text-xs">
        ...
        {viewMode === "projection" && projectionStepState.latestCapture ? (
          <div className="mt-1 text-[11px] font-bold text-amber-700">
            Column {projectionStepState.latestCapture.hd} append: {projectionStepState.latestCapture.value}
          </div>
        ) : null}
      </div>
    </section>
